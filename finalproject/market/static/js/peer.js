...
// Passez par les ACK pour envoyer les pièces manquantes.
...
// Configurez les gestionnaires DataChannel.
Reliable.prototype._setupDC = function() {
  // Gérez différents types de messages.
  var self = this;
  this._dc.onmessage = function(e) {
    var msg = e.data;
...
Reliable.prototype._handleMessage = function(msg) {
  ...
  var data;
  switch (msg[0]) {
    // Aucun découpage n'a été effectué.
    case 'no':
      var message = id;
      if (!!message) {
      ...
      break;
    // Atteint la fin du message.
    case 'end':
      data = idata;

      // Au cas où la fin viendrait en premier.
    ...
    case 'ack':
      ...
      if (!!data) {
        var ack = msg[2];
        // Prenez le plus grand ACK, pour les messages hors service.
        data.ack = Math.max(ack, data.ack);

        // Nettoyez lorsque tous les blocs sont ACQUIS.
      ...
      // If !data, just ignore.
      break;
    // Reçu un morceau de données.
    case 'chunk':
      // Créez une nouvelle entrée s'il n'en existe pas.
      data = idata;
      if (!data) {
      ...
      // Si nous obtenons le morceau que nous recherchons, ACK pour le prochain manquant.
      // Sinon, ACK le même N à nouveau.
      if (n === data.ack[2]) {
      ...
      break;
    default:
      // Cela ne devrait pas arriver, mais il serait logique que le message passe
      // à travers tel quel.
...
// Morceaux BL en messages plus petits.
...
// Envoie ACK N, en attendant le Nième bloc blob pour l'ID de message.
Reliable.prototype._ack = function(id) {
  var ack = this._incoming[id].ack;

  // si ack est la valeur finale, appelez _complete.
...
// Calcule le numéro ACK suivant, en fonction des blocs.
Reliable.prototype._calculateNextAck = function(id) {
  var data = this._incoming[id];
  var chunks = data.chunks;
  for (var i = 0, ii = chunks.length; i < ii; i += 1) {
    // Ce morceau manque !!! Mieux vaut ACK pour cela.
...
// Envoie la prochaine fenêtre de morceaux.
...
//Rassemble un message à partir de morceaux.
